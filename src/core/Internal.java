...
public final class Internal {
  ...
  public static byte[] buildQualifier(final long timestamp, final short flags) {
	  return buildQualifier(timestamp, flags, false);
  }
  
  public static byte[] buildQualifier(final long timestamp, final short flags, final boolean isString) {
    final long base_time;
    if ((timestamp & Const.SECOND_MASK) != 0) {
      ...
      final int qual;
      if(isString) {
    	  qual = (int) (((timestamp - (base_time * 1000) 
    			  << (Const.MS_FLAG_BITS)) | flags) | (Const.MS_FLAG | Const.FLAG_STRING));
      }
      else {
    	  qual = (int) (((timestamp - (base_time * 1000) 
