...
final class IncomingDataPoints implements WritableDataPoints {
  ...
  */
  ...
  private Deferred<Object> addPointInternal(final long timestamp, final byte[] value,
                                            final short flags, final boolean isString) {
    if (row == null) {
    ...
    final byte[] qualifier = Internal.buildQualifier(timestamp, flags, isString);
  ...
  public Deferred<Object> addPoint(final long timestamp, final long value) {
    ...
    return addPointInternal(timestamp, v, flags, false);
  }
  ...
  public Deferred<Object> addPoint(final long timestamp, final String value) {
	if ((timestamp & Const.SECOND_MASK) == 0) {
		throw new IllegalArgumentException("The TimeStamp" + timestamp
                + "with value="+ value +" is not in millisecond. Timestamp should be in milliseconnd to add String value");
	}
	final byte[] v;
	v = Bytes.ISO88591(value+Const.END_OF_STRING);
	final short flags = (short) (v.length - 1);  // Just the length.
	return addPointInternal(timestamp, v, flags, true);
  }
  ...
  public String stringValue(final int i) {
	  return new String(Bytes.fromLong(values[i]),CharsetUtil.ISO_8859_1);
  }
