...
final class RowSeq implements DataPoints {
  ...
  static String extractStringValue(final byte[] values,
		  								final int value_idx,
		  								final byte flags) {
	  int lengthOfVal=values.length-value_idx;
	  String value=new String(values,value_idx,lengthOfVal,CharsetUtil.ISO_8859_1);
	  int idx=value.indexOf(Const.END_OF_STRING);
  ...
  final class Iterator implements SeekableView, DataPoint {
    ...
    private boolean isTimeInMilliSeconds;
    Iterator() {
    }
    ...
    public String stringValue() {
    	if(isString()) {
    		final byte flags = (byte) qualifier;
    		final byte vlen = (byte) ((flags & Const.LENGTH_MASK) + 1);
    		String val = extractStringValue(values, value_index - vlen, flags);
    		value_index = (value_index - vlen)+val.length()+Const.END_OF_STRING.length();
    		return val;
    	}
    	else if(isInteger()) {
